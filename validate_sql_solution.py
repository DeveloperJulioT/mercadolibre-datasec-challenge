...
def validate_sql_solution():
    ...
    expected_customer = "Whitney Ferrero"
    expected_failures = 6

    if not df.empty and df.loc[0, "customer"] == expected_customer and df.loc[0, "failures"] == expected_failures:
        print("\nValidación exitosa: la consulta devuelve el resultado esperado.")
    else:
        print("\nValidación fallida: el resultado no coincide con el esperado.")

    conn.close()

# Bloque de ejecución directa
if __name__ == "__main__":
    validate_sql_solution()
